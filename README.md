...
* 마이닝
	...
	* 서버/클라 : 마이닝 비지니스 로직 개발
		...
		* 서버 : 마이닝 정보 Registe된 클라에게 인스턴스 마이닝 정보 인터벌 주기로 소켓전송
			...
			* 유저ID를 키로 가지고 소켓ID를 값으로 가지는 컨테이너가 있는게 좋을거 같다.
		...
		* 클라 : Slot UI에서 구매버튼 이벤트로 채굴 API 호출하고, Response를 받아 UserInfo와 인스턴스 마이닝 액티브 테이블 업데이트 해주기
			...
			* 클라 : 채굴 정보 데이터 받기
			* 클라 : 마이닝 리스트 정리 : 같은 광물일경우 가장 빠르게 팔려야하는것만 추려서 리스트 만들기, 버튼을 하나 넣어서 전체 리스트 확인할 수 있는 UI만들기, 기본회사는 가장 아랫쪽에 정렬
