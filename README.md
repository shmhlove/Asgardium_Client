...
* 마이닝
	...
	* 서버/클라 : 마이닝 비지니스 로직 개발
		...
		* 클라 : 네트워크 에러일경우 네트워크 모듈에서 처리, 아닐경우 에러 출력 모듈에서 처리
			--
			에러출력모듈에서는 메시지를 큐로 관리하고,
			네트워크 에러일때는 무시하고, 정해진 문구를 출력하도록 하는것과, 디폴트로 에러그대로 출력하는 스위치코드 작성
			--
			네트워크 모듈은 
			재시도 횟수를 인디케이터를 통해 제공
			재시도 횟수가 맥스를 초과하면 팝업으로 알려주고, 재시도 및 종료 처리
			--
			인디케이터 및 알림팝업 오픈시 코루틴을 Resume시킬 방법 고민
			--
